// Vieu Art Studio POS — Service Worker
// Caches the app for offline use

const CACHE_NAME = 'vieu-pos-v1';
const ASSETS = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('fetch', event => {
  // Skip non-GET and Firebase real-time requests
  if (event.request.method !== 'GET') return;
  if (event.request.url.includes('firebaseio.com')) return;
  if (event.request.url.includes('googleapis.com/sheets')) return;
